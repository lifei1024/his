...
public class MedicineInstockServiceImpl  implements IMedicineInstockService {
    ...
    @Override
    public void instock(HttpSession session) {


    }

    @Override
    public List<MedicineInstock> find() {
        String sql = "SELECT * from medicine_instock mi JOIN base_user bu ON mi.instock_userid=bu.user_id\n" +
                "\t\t\t\tJOIN medicine_code mc ON mi.medicine_codeid=mc.code_id";
